...
class BallController:
    def __init__(self, ball: Ball):
        self.ball = ball
        self.ball_angle = 90
        self.ball_angle_rotate_speed = 10

    def get_arrow_points(self):
        size_factor = self.ball.shape.radius / 10

        arrow_length = round(50 * size_factor)
        arrow_sides_length = round(25 * size_factor)

        print(self.ball_angle, arrow_length, radians(self.ball_angle), cos(radians(self.ball_angle)), sin(radians(self.ball_angle)))
    ...
    def rotate_left(self, time_delta: typing.Optional[float] = 1): self.ball_angle - self.ball_angle_rotate_speed * time_delta
    def rotate_right(self, time_delta: typing.Optional[float] = 1): self.ball_angle + self.ball_angle_rotate_speed * time_delta

    def update_rotate_speed(self, new_speed: float): self.ball_angle_rotate_speed = new_speed

    def launch(self, impulse: float):
        self.ball.body.apply_impulse_at_local_point(list(map(lambda point: point * impulse,
                                                             self.get_arrow_points()["end_point"])))